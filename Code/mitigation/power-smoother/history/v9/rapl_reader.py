...
class RaplMonitor:
    ...
    def current(self) -> tuple[float, float]:
        """(t_rel_s, watts) of the most recent sample — for real-dt derivatives.

        Use this instead of current_watts when computing dP/dt: it carries the
        sample's own timestamp so the rate divides by the true ~interval gap,
        not by however fast the caller happens to poll.
        """
        with self._lock:
            return self._samples[-1] if self._samples else (0.0, self._power)
