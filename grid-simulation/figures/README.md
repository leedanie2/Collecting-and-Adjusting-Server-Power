...
| `all_metrics.png` | **Every** metric (all 12 grid + cost) × 3 mitigations in one view, grouped by trust tier. Y-axis is inverted so the lower-is-better convention reads as **up = better**; outliers past ±200% are clipped and labelled. The detector-metrics slot is marked "not yet scored" (detector event metrics aren't computed from the power traces — see below). |
