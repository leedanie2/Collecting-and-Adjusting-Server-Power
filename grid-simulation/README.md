...
The full 3×4 matrix was collected three times on a quiesced server, so every
...
|---|---|---|---|---|---|---|
| 1 | **rampc** | **−68.8 ± 1.1%** | −5.1 ± 0.4% | +6.7 ± 1.1% | +37.4 ± 1.7% | strong smoothing |
| 2 | powersmoother | +0.8 ± 2.2% | −0.4% | +3.5 ± 0.2% | +7.5 ± 0.7% | no measurable smoothing |
| 3 | usagegov | +16.3 ± 5.9% | −0.7 ± 0.2% | +1.4 ± 2.1% | +11.7 ± 0.8% | worsens variability |

Mean ± SD across the three runs. Negative is better for CV/peak/runtime; energy
...
69%, for a +37% energy and +7% runtime cost. The standalone power smoother does
nothing measurable at this timescale, and the slew governor makes variability
worse. The CV bar is tight (±1.1%) even though the raw per-cell runtimes swung
10–19% between runs — the grid-relevant metric is stable; wall-clock time is the
noisy one, which is exactly why three collections were taken.
...
analysis/      Python scoring — grid_metrics.py (per-run metrics),
               ...
               n=3 scoreboard), plot_traces.py
scripts/       build_model.sh, sim_trace.sh, score_all.sh, nrun_pipeline.sh
               (regenerate the n=3 result), trim_auto.py / trim_rampc.py (trace
               prep, see below)
data/traces/   input power traces, named <workload>_<method>.csv (one run)
data/runs/     run{1,2,3}/  the three repeat collections (raw traces + logs)
data/summary/  the n=3 scoreboard: scoreboard.csv, matrix.csv, summary.md
...
results/<run>_r<N>_worst/  per-run metrics.json for the n=3 scoreboard
...
baseline-vs-mitigation comparisons. The whole matrix was collected three times
(`data/runs/run{1,2,3}/`) so the headline scoreboard reports mean ± SD, not a
